...
def interpolate_signal_and_saliency(ts, saliency, interp_points=5000):
    ...
    sal_interp = interp1d(
        x_orig,
        normalize_saliency_0_100(saliency),
        kind="linear",
    )
...
class QualitativeSaliencyPlotter:
    ...
    def plot_hydra_mrsqm_windowshap(self, dataset="GunPoint"):
        ...
        shap_sal = self.windowshap.explain(
            model=hydra,
            x=ts,
            pred_label=hydra_pred,
        )

        x_dense, y_dense, hydra_dense = interpolate_signal_and_saliency(
            ts,
            hydra_sal,
            interp_points=self.interp_points,
        )
        _, _, mrsqm_dense = interpolate_signal_and_saliency(
            ts,
            mrsqm_sal,
            interp_points=self.interp_points,
        )
        _, _, shap_dense = interpolate_signal_and_saliency(
            ts,
            shap_sal,
            interp_points=self.interp_points,
        )
        ...
        fig.suptitle(
            f"{dataset}: HYDRA, MrSQM and WindowSHAP saliency comparison",
            fontsize=13,
        )
